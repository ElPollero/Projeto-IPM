...
const GROUP_NUMBER        = 49;      // Add your group number here as an integer (e.g., 2, 3)
...
const miss_sound = new Audio("miss.mpga");
const nice_sound = new Audio("nice.mpga");
...
function draw()
{
  ...
  {     
    ...
    // Draw all targets
	targets.forEach(t=>t.draw(currentT));
...
function mouseMoved() {
  if(!allowToDraw) return false;
  
  for (let target of targets) {
    let i = target.update(mouseX, mouseY);
...
function createTargets(target_size, horizontal_gap, vertical_gap)
{
  ...
  // Sort labels
  let sortedLabels = legendas.getArray().sort((a,b)=>{
    const cityA = a[1];
    const cityB = b[1];
  ...
  for (var r = 0; r < GRID_ROWS; r++)
  {
    for (var c = 0; c < GRID_COLUMNS; c++)
    {
      ...
      let legendas_index = sortedLabels.indexOf(
          sortedLabels[c + GRID_COLUMNS * r]
      );
      
      let target_id = sortedLabels[legendas_index][0];  
      let target_label = sortedLabels[legendas_index][1];
      
      const targetKey = target_label[1].toLowerCase();
      
      // search city without "é", so it has the same color of "e"
      let targetColor = colorMapping[targetKey.replace("é", "e")];
      
      let target = new Target(target_x, target_y + 40, target_size, target_label, target_id, targetColor);
      targets.push(target);
    }  
  }
}
